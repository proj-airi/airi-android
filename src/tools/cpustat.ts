...
async function parseProcStat(output: string): Promise<TotalCPUStat | undefined> {
  ...
  if (match) {
    return {
      user: Number.parseInt(match[1], 10),
      nice: Number.parseInt(match[2], 10),
      system: Number.parseInt(match[3], 10),
      idle: Number.parseInt(match[4], 10),
      iowait: Number.parseInt(match[5], 10),
      irq: Number.parseInt(match[6], 10),
      softirq: Number.parseInt(match[7], 10),
    }
  }
}
